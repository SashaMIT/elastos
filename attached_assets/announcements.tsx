      ...
      const enrichedItems = data.items.map((item: NewsItem) => {
        ...
        if (!imageUrl) {
          ...
          const sourceDefaults: Record<string, string> = {
            ...
            "Chainwire": "/elastos-essentials-features-3.png"
          };
          imageUrl = sourceDefaults[item.source] || "/elastos-icon.png";
        }
                      ...
                      <CarouselItem key={post.link} className="md:basis-1/2 lg:basis-1/3 pl-4">
                        ...
                        >
                          ...
                          <div className="relative h-48 overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent opacity-70 z-10"></div>
                            <img 
                              src={post.imageUrl || "/elastos-icon.png"} 
                              alt={post.title}
                              className="w-full h-full object-cover object-center transition-transform duration-500 group-hover:scale-105"
                            />
