...
export async function registerRoutes(server: FastifyInstance) {
  ...
  server.get("/api/elastos-news", async (request, reply) => {
    try {
      ...
      // Fallback sample data in case the feed is not accessible
      let items = [
        {
      ...
      try {
        ...
        if (parsedFeed && parsedFeed.items && parsedFeed.items.length > 0) {
          items = parsedFeed.items.map(item => {
            ...
            let imageUrl = "/images/placeholder-image.jpg";
            if (item.content) {
              const imgMatch = item.content.match(/<img[^>]+src="([^">]+)"/i);
              if (imgMatch && imgMatch[1]) {
                imageUrl = imgMatch[1];
        ...
        console.warn("Could not fetch RSS feed, using fallback data:", feedError);
      }
      ...
      return reply.send({
        total: items.length + parsedOffset, // Estimate total
        offset: parsedOffset,
        limit: parsedLimit,
        items: items
      });
